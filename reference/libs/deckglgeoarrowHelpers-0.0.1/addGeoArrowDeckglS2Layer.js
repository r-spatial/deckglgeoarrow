addGeoArrowDeckglS2Layer = function(map, opts) {
  ...
  fetch(data_fl.href)
    ...
    .then(arrow_table => {
      ...
      let s2layer = s2Layer(map, opts, arrow_table);

      // does the mapboxoverlay already have layer(s)?
      if (deckoverlay._props.layers.length ===  0) {
        deckoverlay.setProps({ layers: [s2layer] })
      } else {
        let lrs = deckoverlay._props.layers.concat(s2layer);
...
s2Layer = function(map, opts, table) {
